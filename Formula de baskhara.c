...
int main ()
{
    ...
    if ( coeficienteA == 0 ){   // Ver se o o coeficienteA nao é igual a zero antes de calcular o delta!
        printf("Coeficiente A não pode ser zero. A equação não é quadrática.\n");
        return 1;
    }

    delta = pow (coeficienteB, 2) - 4 * coeficienteA * coeficienteC;    // Calcular o delta
    ...
    } else if ( delta == 0 ){   // Se delta for igual a 0:
        x = -coeficienteB / ( 2 * coeficienteA );

        printf("X = %.4lf", x);

    } else if ( delta < 0 ){    // se delta for menor que 0:
